{
	"translatorID": "0e2235e7-babf-413c-9acf-f27cce5f059d",
	"label": "MODS mod by Kai",
	"creator": "Simon Kornblith and Richard Karnesky, mod by Kai",
	"target": "xml",
	"minVersion": "2.1.9",
	"maxVersion": "",
	"priority": 50,
...
// Namespace array for using ZU.xpath
	var ns = "", //emptied by kai. before: http://www.loc.gov/mods/v3
	xns = {"m":ns};
...
function doExport() {
	...
	var citekeys = new Object(); //by kai
	var item;
	while(item = Zotero.nextItem()) {
		...
		/** mods by Kai **/
		if(item.title) {
			var citekey = doc.createElement("citeKey");
			// create a unique citation key
			var citekeyID = buildCiteKey(item, citekeys);

			var citekeyContent = doc.createTextNode(citekeyID);
			citekey.appendChild(citekeyContent);
			mods.appendChild(citekey);
		}
